...
const UploadModal: React.FC<UploadModalProps> = ({
    ...
    const handleUpload = async () => {
        ...
        try {
            const formData = new FormData();
            formData.append('files', file); // Обратите внимание на 'files' (множественное число)
            formData.append('knowledge_base_name', kbName);
            formData.append('project_name', projectName);

            const response = await fetch('/rag_service/upload_files/', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${state.token}`
                },
        ...
        <div className={styles.modalOverlay}>
            <div className={styles.modalContent}>
                ...
                <div className={styles.modalFooter}>
                    ...
                    <button
                        onClick={handleUpload}
                        className={`${styles.uploadButton} ${isUploading ? styles.uploadButtonLoading : ''}`}
                        disabled={!kbName || !file || isUploading}
                    >
                        {isUploading ? (
                            <>
